...
class WebsiteSurveyExtend(http.Controller):
    # HELPER METHODS #
    ...
    def _check_bad_cases(self, survey, token=None):
        ...
        # Get reviewer id search by token，id it is appraisal
        if survey.auth_required and survey.is_appraisal:
            survey_input = request.env['survey.user_input'].search([('token', '=', token)])
            if survey_input:
                reviewer_user_id = survey_input.reviewer_id.user_id.id

                if not reviewer_user_id == request.env.user.id:
                    return request.render("survey.auth_required", {'survey': survey, 'token': token})
            else:
                return request.render("survey.auth_required", {'survey': survey, 'token': token})
    ...
    def start_survey(self, survey, token=None, **post):
        ...
        if user_input.state == 'new':  # Intro page
            # s1 = copy.copy(survey)
            # s1.title = survey.title + ' for ' + user_input.reviewer_id.name

            data = {'survey': survey, 'page': None, 'token': user_input.token, 'appraisal_person': 'for ' + user_input.appraisal_id.emp_id.name}
            return request.render('survey.survey_init', data)
        else:
